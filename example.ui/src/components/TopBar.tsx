...
export default function TopBar({ onExport }: TopBarProps) {
  return (
    <div className="h-12 bg-canvas border-b border-hairline flex items-center justify-between select-none px-3 shrink-0 shadow-sm relative z-40">
      {/* Left Menu & Logo */}
      <div className="flex items-center h-full">
        <div className="flex items-center gap-2 px-2 mr-6">
          <div className="w-5 h-5 bg-gradient-to-br from-primary to-accent-soft rounded-md flex items-center justify-center shadow-sm">
            <Video className="w-3 h-3 text-white" />
          </div>
          <span className="text-[13px] font-bold text-ink tracking-wide">Palmier</span>
        </div>
        
        <div className="flex items-center gap-0.5">
          {['File', 'Edit', 'Timeline', 'View', 'Window', 'Help'].map((item) => (
            <button key={item} className="px-3 py-1.5 text-[12px] font-medium text-body hover:text-ink hover:bg-surface-card-strong rounded-md transition-colors">
              {item}
            </button>
          ))}
        </div>
