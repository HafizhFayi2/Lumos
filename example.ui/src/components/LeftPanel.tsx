...
export default function LeftPanel() {
  const { setClips, duration } = useEditor();
  ...
  const handleAddMedia = (item: typeof mediaItems[0]) => {
    const isAudio = item.type === 'audio';
    
    setClips(prev => {
      // Find track with fewest clips
      const track = isAudio ? 'A1' : 'V1'; 
      // Very naive logic, just put it at the end of the timeline or at a random spot for demo
      const start = Math.random() * (duration / 2);
    ...
    <div className="w-[280px] bg-surface-card border-r border-hairline flex flex-col shrink-0">
      ...
      <div className="flex-1 overflow-y-auto px-2 pb-2">
        <div className="grid grid-cols-2 gap-2">
          {mediaItems.map(item => (
            <div key={item.id} onClick={() => handleAddMedia(item)} className="group flex flex-col gap-1.5 cursor-pointer">
              <div className="aspect-video bg-surface-card-strong border border-hairline rounded-lg overflow-hidden relative group-hover:border-primary/50 group-hover:shadow-[0_4px_20px_rgba(59,130,246,0.15)] group-hover:scale-[1.02] transition-all duration-300">
                {/* Mock Thumbnail */}
